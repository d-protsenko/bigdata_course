dataclients <- scan("bank_random.txt")
...
trust <- A[, 3]
n <- length(salary)
plot(salary, trust, type = "n", main = "Зависимость между зарплатой + возрастом и платёжеспособностью")
for (i in 1:n)
{
...
B <- as.data.frame(A)
mymodel <- glm(formula = Trust ~ Salary, family = binomial(logit), data = B)
summary(mymodel)
Teta <- coef(mymodel)
...
newSalaryPlusAge <- c(15, 16, 17, 18, 19)
